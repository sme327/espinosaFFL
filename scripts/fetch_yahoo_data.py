#!/usr/bin/env python3
"""
fetch_yahoo_data.py — Espinosa FFL Clubhouse

Scrapes Yahoo Fantasy Football historical data for the Espinosa FFL.
Espinosa FFL is a snake redraft league (no auction, no keepers) that still
runs on Yahoo — this scraper keeps the Clubhouse's archive in sync with it.

Writes straight into data/source/, which scripts/normalize_league.py reads
to regenerate app/data/league.json. After scraping, run:

    python3 scripts/normalize_league.py

Usage:
    python3 scripts/fetch_yahoo_data.py                  # all years
    python3 scripts/fetch_yahoo_data.py --year 2024      # single year
    python3 scripts/fetch_yahoo_data.py --section draft  # specific section only

New season checklist: once a season wraps on Yahoo, add its year to YEARS
and its (game_code, league_id) pair to LEAGUE_IDS below before scraping.
"""
...
# Reuse AND project cookies if local ones don't exist yet — this and "A New
# Dynasty" are the same real-world Yahoo account's two leagues. This repo lives
# at "<Fantasy Football Leagues>/Espinosa FFL Clubhouse/Clubhouse/", so the
# leagues directory is two levels up, and AND's museum repo is "A New Dynasty/Museum/".
_local_cookies = ROOT / ".yahoo_cookies.json"
_and_cookies   = ROOT.parents[1] / "A New Dynasty" / "Museum" / ".yahoo_cookies.json"
