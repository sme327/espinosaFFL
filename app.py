...
if HERO_PATH.exists():
    ...
    st.markdown(
        """<div style="text-align:center;padding:1rem 0 0.25rem;">
            <div style="font-family:'Fredoka One',cursive;font-size:2.5rem;
                        color:#0A5EA8;line-height:1.1;">
                Espinosa Fantasy Football Clubhouse
            </div>
            <div style="font-family:'Nunito',sans-serif;font-weight:700;
                        font-size:1rem;color:#666;margin-top:0.35rem;">
                One League &nbsp;&bull;&nbsp; One Family &nbsp;&bull;&nbsp; Countless Memories
            </div>
    ...
    st.markdown(
        """<div class="hq-hero-text">
            ...
            <div class="hq-hero-tagline-text">
                One League &nbsp;&bull;&nbsp; One Family &nbsp;&bull;&nbsp; Countless Memories
            </div>
            <div class="hq-hero-sub-text">
                Champions &bull; Rivalries &bull; Traditions &bull; Draft Day
            </div>
        </div>""",
        unsafe_allow_html=True,
...
# ── FOOTBALL DIVIDER ───────────────────────────────────────────────────────
st.markdown(
    '<div class="hq-divider-football">🏈</div>',
    unsafe_allow_html=True,
)
...
with champ_col:
    st.markdown(
        f"""<div class="hq-champ-featured" style="border-color:{champ_color};
                box-shadow:0 12px 56px rgba(214,163,25,0.16),
                           0 4px 24px rgba(0,0,0,0.07);">
            <!-- gold top bar is ::before -->
            <div class="hq-confetti-tl">🎊 🎉 🎊</div>
            <div class="hq-confetti-tr">🎊 🎉 🎊</div>

            <span class="hq-champ-featured-trophy">🏆</span>

            <div class="hq-champ-featured-label">
                ✦ &nbsp; {int(latest['season'])} League Champion &nbsp; ✦
            </div>

            <div class="hq-champ-featured-name" style="color:{champ_color};">
                {champ_emoji} {champ_mgr}
            </div>
            <div class="hq-champ-featured-team">{latest['champion_team']}</div>
            <div class="hq-champ-featured-score">
                {latest['champion_score']:.1f} – {latest['runner_up_score']:.1f}
                &nbsp;&bull;&nbsp; defeated {latest['runner_up_manager']}
            </div>
            <div class="hq-champ-featured-tagline">"{tagline}"</div>

            <div style="text-align:center;margin-top:1.25rem;">
                <a href="/champions" target="_self"
                   style="display:inline-block;font-family:'Nunito',sans-serif;
                          font-weight:800;font-size:0.82rem;color:white;
                          background:{champ_color};padding:9px 22px;
...
with col_lb:
    ...
    for i, (_, row) in enumerate(at_standings.iterrows()):
        ...
        rows_html += f"""
        <div class="hq-lb-row">
            <div style="font-size:1.35rem;min-width:1.8rem;text-align:center;">{medal}</div>
            <div class="hq-lb-avatar" style="background:{color};width:36px;height:36px;
                     border-radius:50%;display:flex;align-items:center;
                     justify-content:center;font-size:1.1rem;flex-shrink:0;">{emoji}</div>
            <div class="hq-lb-info" style="flex:1;min-width:0;">
                <div class="hq-lb-name" style="font-family:'Nunito',sans-serif;
                     font-weight:800;font-size:0.9rem;color:#222;">{mgr}</div>
                <div class="hq-lb-record" style="font-family:'Nunito',sans-serif;
                     font-size:0.7rem;color:#AAA;">
                    {row['wins']}–{row['losses']} &nbsp;·&nbsp; {row['win_pct']:.0%}
                </div>
            </div>
            <div style="font-size:0.95rem;flex-shrink:0;">{trophy}</div>
        </div>"""

    st.markdown(
        f"""<div style="background:#FFFDF8;border-radius:18px;padding:1.4rem 1.3rem;
                    box-shadow:0 4px 20px rgba(0,0,0,0.07);
                    border-top:5px solid #D6A319;height:100%;">
            <div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:0.75rem;">
                <span style="font-size:1.5rem;">🏟️</span>
                <div>
                    <div style="font-family:'Fredoka One',cursive;font-size:1.1rem;color:#333;">
                        All-Time Standings
                    </div>
                    <div style="font-family:'Nunito',sans-serif;font-size:0.7rem;
                                color:#AAA;font-weight:700;">Regular season wins</div>
                </div>
            </div>
            {rows_html}
            <div style="text-align:center;margin-top:0.9rem;">
                <a href="/manager_profiles" target="_self"
                   style="font-family:'Nunito',sans-serif;font-weight:800;font-size:0.76rem;
                          color:#0A5EA8;text-decoration:none;border-bottom:2px solid #D9D7CF;
                          padding-bottom:1px;">
                    Open Locker Room →
                </a>
            </div>
...
with col_rival:
    ...
    if streak and streak_ct:
        ...
        streak_html = (
            ...
            f'color:{sc};font-weight:800;text-align:center;margin-top:0.3rem;">'
            f"🔥 {streak} on a {streak_ct}-game streak"
            f"</div>"
        )

    # Split-color helmet row
    st.markdown(
        f"""<div style="background:#FFFDF8;border-radius:18px;padding:1.4rem 1.3rem;
                    box-shadow:0 4px 20px rgba(0,0,0,0.07);
                    border-top:5px solid {ca};height:100%;position:relative;overflow:hidden;">
            <!-- split bg -->
            <div style="position:absolute;top:0;left:0;bottom:0;width:50%;
                        background:{la};opacity:0.18;border-radius:18px 0 0 18px;"></div>
            <div style="position:absolute;top:0;right:0;bottom:0;width:50%;
                        background:{lb_col};opacity:0.18;border-radius:0 18px 18px 0;"></div>
            ...
            <div style="position:relative;">
                <div style="display:flex;align-items:center;gap:0.4rem;margin-bottom:0.8rem;">
                    ...
                    <div>
                        ...
                        <div style="font-family:'Nunito',sans-serif;font-size:0.7rem;
                                    color:#AAA;font-weight:700;">Today's featured matchup</div>
                    </div>
                </div>
                ...
                <div style="display:flex;align-items:center;justify-content:space-between;
                    ...
                    <div style="flex:1;text-align:center;">
                        <div style="width:48px;height:48px;border-radius:50%;background:{ca};
                                    display:flex;align-items:center;justify-content:center;
                                    font-size:1.5rem;margin:0 auto 0.25rem;">{ea}</div>
                    ...
                    <div style="text-align:center;flex-shrink:0;padding:0 0.5rem;">
                        <div style="font-family:'Fredoka One',cursive;font-size:2.2rem;color:#333;">
                            {ta}–{tb}
                        </div>
                        <div style="font-family:'Nunito',sans-serif;font-size:0.58rem;
                                    font-weight:800;text-transform:uppercase;
                                    letter-spacing:1.5px;color:#CCC;">All Time</div>
                    </div>
                    <!-- Manager B -->
                    <div style="flex:1;text-align:center;">
                        <div style="width:48px;height:48px;border-radius:50%;background:{cb};
                                    display:flex;align-items:center;justify-content:center;
                                    font-size:1.5rem;margin:0 auto 0.25rem;">{eb}</div>
                        <div style="font-family:'Fredoka One',cursive;font-size:1.3rem;
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.4rem;margin-top:0.6rem;">
                    <div style="background:rgba(255,255,255,0.7);border-radius:8px;
                        ...
                        <div style="font-family:'Nunito',sans-serif;font-size:0.57rem;
                                    font-weight:800;text-transform:uppercase;
                                    letter-spacing:1px;color:#AAA;">Reg. Season</div>
                        <div style="font-family:'Fredoka One',cursive;font-size:1.1rem;
                                    color:#333;">{rs_a}–{rs_b}</div>
                    </div>
                    <div style="background:rgba(255,255,255,0.7);border-radius:8px;
                                padding:0.4rem;text-align:center;">
                        <div style="font-family:'Nunito',sans-serif;font-size:0.57rem;
                                    font-weight:800;text-transform:uppercase;
                                    letter-spacing:1px;color:#AAA;">Playoffs</div>
                        <div style="font-family:'Fredoka One',cursive;font-size:1.1rem;
                                    color:#333;">{po_a}–{po_b}</div>
                    </div>
                </div>

                <div style="text-align:center;margin-top:0.85rem;">
                    <a href="/rivalries" target="_self"
                       style="font-family:'Nunito',sans-serif;font-weight:800;font-size:0.76rem;
                              color:#0A5EA8;text-decoration:none;border-bottom:2px solid #D9D7CF;
                              padding-bottom:1px;">
                        View Rivalry Arena →
...
# ── DIVIDER ────────────────────────────────────────────────────────────────
st.markdown(
    '<div class="hq-divider-football" style="margin:1.6rem 0;">🏈</div>',
    unsafe_allow_html=True,
)
...
with col_bull:
    items_html = "".join(
        f"""<div class="hq-bulletin-item">
            <span class="hq-bulletin-pin">{icon}</span>
            <span>{text}</span>
        </div>"""
...
for _, row in champions.iterrows():
    ...
    items_html += f"""
    <div class="hq-trophy-item">
        <span class="hq-trophy-item-icon">{icon}</span>
        <div class="hq-trophy-item-year">{int(row['season'])}</div>
        <div class="hq-trophy-item-name" style="color:{color};">{mgr}</div>
        <div class="hq-trophy-item-team">{row['champion_team']}</div>
    </div>"""

# "Coming soon" slot for next season
items_html += """
...
for col, room in zip(explore_cols, ROOMS):
    with col:
        ...
        href_attr   = f'href="{room["href"]}"' if not room["soon"] else 'href="#"'
        target_attr = 'target="_self"' if not room["soon"] else ""
        arrow       = "" if room["soon"] else f'<div class="hq-explore-arrow" style="color:{room["color"]};">→</div>'
        soon_badge  = '<div style="font-family:Nunito,sans-serif;font-size:0.62rem;font-weight:800;text-transform:uppercase;letter-spacing:1px;color:#CCC;margin-top:0.4rem;">Coming Soon</div>' if room["soon"] else ""
        col.markdown(
            f"""<a {href_attr} {target_attr} class="hq-explore-card{soon_cls}">
                <span class="hq-explore-icon">{room['icon']}</span>
                <div class="hq-explore-room-title" style="color:{'#AAA' if room['soon'] else '#222'};">
                    {room['title']}
                </div>
                <div class="hq-explore-room-desc">{room['desc']}</div>
                {arrow}
                {soon_badge}
            </a>""",
            unsafe_allow_html=True,
...
st.markdown(
    """<div style="padding:2.5rem 0 1.5rem;text-align:center;">
        <div style="font-family:'Nunito',sans-serif;font-size:0.7rem;color:#CCC;
                    font-weight:700;letter-spacing:2px;text-transform:uppercase;">
            Espinosa Fantasy Football Clubhouse &bull; Est. 2023 &bull; All Rights Reserved to the Family
        </div>
    </div>""",
    unsafe_allow_html=True,
)
